...
> The single source of truth for SKC's visual design — the **tokens** (§2) and the
> **pattern library** (every reusable UI pattern: markup, slots, rules). CLAUDE.md
> routes here for any visual, styling, or layout work, and this is the spec the
> Next.js rebuild adopts. Promoted from the locked v32 design system; the
> originating audit is archived at `archive/design-system-audit.md`.

> **Adoption status — read this.** The patterns here are the **target** and the
> contract for the rebuild. In the current prototype they are fully wired only on
> the newest redesigned screens (labor-efficiency, table-turns, roi-proof,
> kitchen-speed); other screens still use the legacy per-feature prefixes in each
> pattern's **Migration map**. Treat those maps as a to-do list, not a description
> of the present. Known code-vs-doc deltas: the tab bar is still `skc-tab-*` (not
> `tab-*`); `doctrine-line` / `aud-badge` / `page-hdr` aren't universal yet; and two
> output-type tag systems coexist — static `tag-*` (hand-authored) and runtime
> `ot-*` via `getOutputLabel()` (§9). When unsure, copy from a screen that already
> uses the canonical class.

## How to use this doc

1. Use the **tokens** (§2) for every value — never a raw hex.
2. **Read the principles** (§1) before designing anything.
3. **Pick the pattern** from the index below, **copy its markup**, adjust slots,
   apply modifier classes.
4. **Use the canonical vocabulary & voice** (§15) for all user-facing copy.
...
## 2. Tokens (literal reference)

Every value in the app references a CSS custom property. These are the **exact**
definitions from `shared/styles.css` — copy the variable name, never the hex.

### Color — dark palette (`:root`)

| Token | Value | Use |
|---|---|---|
| `--bg` | `#09090B` | App background |
| `--surface` | `#111113` | Default surface |
| `--card` | `#18181B` | Card background |
| `--hover` | `#1F1F23` | Hover / nested surface |
| `--border` | `#27272A` | Default border |
| `--border-s` | `#3F3F46` | Stronger border |
| `--t1` | `#FAFAFA` | Primary text |
| `--t2` | `#A1A1AA` | Secondary text |
| `--t3` | `#71717A` | Tertiary text / labels |
| `--green` | `#10B981` | Fine / safe / **verified** |
| `--amber` | `#F59E0B` | Watch / review / **estimated** / open |
| `--red` | `#EF4444` | Act now / blocked / **unavailable** |
| `--blue` | `#3B82F6` | Informational / heuristic / sim / active |

Each semantic color has a tint pair: `--<c>-d` (≈8% fill) and `--<c>-b` (≈20%
border) — e.g. `--green-d: rgba(16,185,129,.08)`, `--green-b: rgba(16,185,129,.2)`.

### Color — light palette (`.light` overrides)

The same token names, re-pointed: `--bg #F7F8FA`, `--surface`/`--card #FFFFFF`,
`--hover #F3F4F6`, `--border #E5E7EB`, `--border-s #D1D5DB`, `--t1 #111827`,
`--t2 #6B7280`, `--t3 #9CA3AF`, `--green #0F766E`, `--blue #2563EB`,
`--amber #D97706`, `--red #DC2626` (tints at ≈7% / ≈18%).
...
| `--mono` | `'Roboto Mono','IBM Plex Mono','SFMono-Regular',Consolas,monospace` |
| `--r` / `--r-md` / `--r-lg` | `6px` / `8px` / `12px` |
| `--sw` | `210px` (sidebar width) |
| `--th` | `52px` (topbar height) |

### Theming

`:root` defines the **dark** palette; **`.light`** re-points the same tokens to the
light ramp, so theming is a single class swap (`toggleTheme()` toggles `.light` on
`<body>`). Note: the demo shell currently ships with **`.light` applied by default**
(`<body class="… light">`), so what you see on load is the light palette even though
dark is the CSS default.

**Rules:**
- Never use a hex value directly in markup — always reference the token.
- Color = meaning, mapped to doctrine: **green = verified**, **amber = estimated /
  open**, **blue = heuristic / sim / active**, **red = unavailable / blocked**.
  Don't decorate with a color; use it only because the state warrants it.
- Modifier classes (`is-warn`, `is-good`, `is-bad`) carry the semantic, not inline
  color.
- Adding a token: define it in **both** `:root` and `.light` in `shared/styles.css`,
  then reference it — never inline the value.
...
Every term that had multiple spellings in the audit — locked to one. The **voice**
rules govern tone; the **vocabulary** tables govern exact wording.

### Voice & register

All user-facing copy (captions, labels, headlines, tooltips, empty states, buttons)
reads **professional, polished, confident**. Favor measured, sophisticated phrasing
over terse or casual.

- **Optimistic and assured, never apologetic or scarcity-driven.** Frame value as
  what the operator gains ("a clear return on your investment"), not what they
  barely avoid losing. Never imply they're scraping by or making a marginal call.
- **Don't surface the platform's price** in value copy. Convey worth through
  outcomes and ROI language; don't anchor on the subscription cost (avoid "covers
  your $349/mo").
- **Keep the honesty doctrine intact while elevating the register.** Ranges stay
  ranges, modeled stays modeled. "A grounded range, not a promise" is good; "even
  the low end covers your cost" is not.
...
*Locked v32, promoted to the spec root. Changes to canonical patterns require updating this doc and logging a line in `CHANGELOG.md`.*
