...
│   ├── data.js                 top-level data fixtures (MENU_DATA, LE_CO_*, ...)
...
│   ├── server-coaching.js      Server Coaching renderer + showLaborSubpage wrap
...
│   ├── labor-efficiency.html   screen-labor-efficiency (6 subpages incl. Server Coaching)
...
3. Loader dynamically injects `<script>` tags for data → core → controllers → server-coaching → tooltips (this load order matters — `server-coaching.js` wraps `showLaborSubpage` from `controllers.js`).
...
3. `docs/data-sources.md` — Toast vs. operator vs. inferred. The Server Coaching axis-state model is here too.
